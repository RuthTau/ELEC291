...
CHANGE_HOUR   equ P0.3
CHANGE_MINUTE equ P0.6
...
;Alarm components
set_alarm_hours: ds 1 
set_alarm_minutes: ds 1 
;Clock components
hours_clock: ds 1 
minutes_clock: ds 1 
seconds_clock: ds 1 
...
main:
    ...
    Set_cursor(2,1)
    Send_Constant_String(#set_alarm)
    setb half_seconds_flag
	;mov BCD_counter, #0x00
	mov hours_clock, #0x12 
	mov minutes_clock, #0x58 
	mov seconds_clock, #0x00 
	mov set_alarm_hours, #0x12
	mov set_alarm_minutes, #0x02
...
loop_b:
    ...
    Send_Constant_String(#set_alarm)
	;Display_BCD(set_alarm_hours) ; This macro is also in 'LCD_4bit.inc'
	...
	Display_BCD(set_alarm_minutes) ; This macro is also in 'LCD_4bit.inc'
	
	;display clock
	Set_Cursor(1, 1)     ; the place in the LCD where we want the BCD counter value
	Send_Constant_String(#Initial_Message)
	;Display_BCD(hours_clock) ; This macro is also in 'LCD_4bit.inc'
	Set_Cursor(1, 6)     ; the place in the LCD where we want the BCD counter value
	Display_BCD(hours_clock) ; This macro is also in 'LCD_4bit.inc'
	;Display_BCD(minutes_clock) ; This macro is also in 'LCD_4bit.inc'
	...
	Display_BCD(seconds_clock) ; This macro is also in 'LCD_4bit.inc'
...
hr_inc_button:
	...
	da a 
	mov hours_clock, a 
	cjne a, #0x13, min_btn_clk
	mov hours_clock, #0x01 
	sjmp min_btn_clk 

min_btn_clk:
	setb CHANGE_MINUTE 
	jnb CHANGE_MINUTE, min_inc_button 
